package com.github.cao.awa.trtr.database.file.system.adapter.region;

import com.github.cao.awa.trtr.database.file.system.adapter.*;
import com.github.cao.awa.trtr.database.file.system.adapter.data.*;
import com.github.cao.awa.trtr.database.file.system.adapter.index.*;
...
public class AdapterDataRegion {
    ...
    public AdapterDataRegion(File base) {
        this.baseFile = base;
        this.data = EntrustParser.trying(() -> new AdapterDataAccessor(new RandomAccessFile(
                this.baseFile + "/page.page",
                "rw"
        )));
        this.index = EntrustParser.trying(() -> new AdapterIndexAccessor(new RandomAccessFile(
    ...
    private void writeToEnd(byte[] bytes) throws IOException {
        long order = insertTo();
    ...
    private BiOption<Long> availableSize(long dataOrder) throws IOException {
        if (dataOrder == 0) {
            ...
            if (startPos == 0) {
                ...
                endPos = index.adapterReadLong(dataOrder);
            }
