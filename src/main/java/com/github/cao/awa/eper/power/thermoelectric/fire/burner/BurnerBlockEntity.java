...
import com.github.cao.awa.eper.properties.*;
...
public class BurnerBlockEntity extends LootableContainerBlockEntity implements BlockEntityTicker<BurnerBlockEntity> {
    ...
    public BurnerBlockEntity(BlockPos pos, BlockState state) {
        super(EperBlockEntityType.BURNER, pos, state);
        this.heatConductor = new HeatConductor(150);
    ...
    public static Map<Item, Integer> createFuelHeatMap() {
        ...
        addHeat(map, Blocks.COAL_BLOCK, 1400);
    ...
    private static void addHeat(Map<Item, Integer> fuelHeats, ItemConvertible item, int fuelTime) {
    ...
    public void readNbt(NbtCompound nbt) {
        ...
        properties.putInt("heat", nbt.getInt("heat"));
    ...
    protected void writeNbt(NbtCompound nbt) {
        ...
        nbt.putInt("heat", properties.getIntOrDefault("heat", 0));
        if (lastBurning != null) {
    ...
    public void tick(World world, BlockPos pos, BlockState state, BurnerBlockEntity blockEntity) {
        if (burn()) {
            if (! state.get(Burner.LIT)) {
                world.setBlockState(pos, state.with(Burner.LIT, true), 2);
            }
            tickBurning(world, pos, state);
        } else {
            if (state.get(Burner.LIT)) {
                world.setBlockState(pos, state.with(Burner.LIT, false), 2);
            }
        }
        markDirty();
    }
    ...
    public void tickBurning(World world, BlockPos pos, BlockState state) {
        ...
        if (targetState.getMaterial() == Material.WOOD) {
            properties.updateDouble("burned", source -> {
                int heat = properties.getIntOrDefault("heat", 0);
        ...
        heatConductor.setSelfHeat(properties.getIntOrDefault("heat", 27));
        heatConductor.heat(world, pos, state, HeatConductionType.TO_MAX);
    }
    ...
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        if (isBurning()) {
            ...
            entity.damage(new DamageSource("eper.burner.hot"), properties.calculateFloat("heat", heat -> heat > 0F, // Is heat higher than zero
    ...
    public void tickClient(World world, BlockPos pos, BlockState state, BurnerBlockEntity burnerBlockEntity) {
        if (state.get(Burner.LIT) && world.getBlockState(pos.up()).isAir()) {
