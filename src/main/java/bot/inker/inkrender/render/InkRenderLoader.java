...
public class InkRenderLoader implements ModelResourceProvider, Function<ResourceManager, ModelResourceProvider> {
    ...
    protected UnbakedModel loadModelResource(Identifier identifier, ModelProviderContext modelProviderContext, ModelTransformation transform) {
        ...
        if (identifier.getPath().endsWith(".obj")) {
            Optional<InkResourceLoader> resource = InkerRender.resourceService().findModel(identifier.toString());
            if (resource.isEmpty()) {
                return null;
            }
            return loadModel(resource.get(), transform);
        }
    ...
    public InkRenderUnbakedModel loadModel(InkResourceLoader resourceLoader, ModelTransformation transform) {
        ...
        try {
            Obj obj = ObjUtils.convertToRenderable(ObjReader.read(new InputStreamReader(resourceLoader.openObj(), StandardCharsets.UTF_8)));
            return new InkRenderUnbakedModel(resourceLoader, ObjUtils.triangulate(obj), loadMTL(resourceLoader, obj.getMtlFileNames()), transform);
        } catch (Exception e) {
            logger.error("Failed to load obj model", e);
            return null;
        }
    }

    public Map<String, Mtl> loadMTL(InkResourceLoader resourceLoader, List<String> names) {
        return names.stream().flatMap(UncheckUtil.cast(task -> MtlReader.read(resourceLoader.openMtl(task)).stream())).collect(Collectors.toMap(Mtl::getName, Function.identity()));
    }
